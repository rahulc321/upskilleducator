...
class HomePageController extends Controller
{
    ...
    public function subscribe(Request $request)
    {
        try {
            ...
            $checkSus = Subscribe::where('email',$request->subscribe_email)->first();

            if($checkSus){
                Session::flash('error', __('You have Already Suscriber!!'));
    ...
    {    
        ...
        //$userEmail = 'rorin31259@bpghmag.com';
        $userEmail = 'upskilleducator@gmail.com';
                             
        

         \Mail::send('emails.query.index',$data, function($message) use ($userEmail)
                    {
                        $message->to($userEmail);
                        $message->subject('Message from Query');
            ...
            \Mail::raw('Thankyou for your message. We will contact you soon..', function ($message)  use ($email2){
            $message->to($email2);
             $message->subject('Message from Query');
            });
        ...
        // return view('emails.query.index', $data);
    }
}
